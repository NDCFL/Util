...
public class UserController  {
    ...
    public Message updateBossInfo(UserVo userVo) {
        try{
            UserVo userVo1=userService.getById(userVo.getId());
            if(userVo1.getName().equals(userVo.getName())){//本人修改，昵称不变
                userService.update(userVo);
                return Message.success("资料修改成功！");
            }
            Long cnt= userService.checkName(userVo.getName());
            if(cnt>0){
    ...
    @RequestMapping("test")
    @ResponseBody
    public Message test(String name) {
        try{
